...
public class ClockWidgetSix extends AppWidgetProvider {
    public void onReceive(Context context, Intent intent) {
        String act = intent.getAction();
        int flags = 0;

        if (AppWidgetManager.ACTION_APPWIDGET_UPDATE.equals(act)) {
            RemoteViews clockView = new RemoteViews(context.getPackageName(), R.layout.analog_clock_6);
            ...
            clockView.setOnClickPendingIntent(R.id.analog_clock_6, PendingIntent.getActivity(context, 0, clockIntent, flags));
