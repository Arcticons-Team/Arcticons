    ...
    def parse_existing(self,block_query,path):  
        ...
        with open(path, 'r', encoding="utf8") as existing_file:
            ...
            for req in existing_requests:
                ...
                self.apps[element_info['ComponentInfo']]['count'] = int(element_info.get('count',1)) if element_info.get('count',1) is not None else 1
                self.apps[element_info['ComponentInfo']]['senders'] = []
        # Writing dictionary to a file in JSON format
        with open("output.json", "w") as file:
            json.dump(self.apps, file, indent=4)  # 'indent=4' makes it pretty-printed
...
class EmailDownloader:
    def __init__(self, server, username, password, folder="INBOX", save_path="./emails", trash_folder="[Gmail]/Trash"):
        self.server = server
        self.username = username
        self.password = password
        self.folder = folder
        self.trash_folder = trash_folder  # Define the Trash folder name for your email provider
    ...
    def fetch_emails(self, search_criteria="UNSEEN", limit=1):
        result, data = self.connection.search(None, search_criteria)
        if result != "OK":
